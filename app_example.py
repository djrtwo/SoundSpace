...
def myTest():
    ...
    initOSCServer(mode=2) # takes args : ip, port, mode --> 0 for basic server, 1 for threading server, 2 for forking server
    ...
    try:
        while 1:
            sendOSCMsg("/check", [i, 'face']) 
            i = i+1
            # create and send a bundle
            # bundle = createOSCBundle("/check")
            ...
            time.sleep(1) # you don't need this, but otherwise we're sending as fast as possible.
